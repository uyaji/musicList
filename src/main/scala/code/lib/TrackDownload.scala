...
object TrackDownload {
  def download(id: Long): Box[LiftResponse] = {
    ...
    val attachment = "attachment; filename=" + MimeUtility.encodeWord(track.filename.get, "ISO-2022-JP", "B")
    val content = track.mimetype.get + "; charset=UTF-8"
    val headers = ("Content-Type" -> "text/plain; charset=UTF-8") :: ("Content-length" -> track.trackatach.get.length.toString) ::("Content-disposition" -> attachment) :: Nil
    Full(
